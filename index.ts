...
const normalizeData = (unnormalizedData: Input[]): Output => {
	let result: Output = {
		byId: {},
		allIds: []
	}
	...
	for (let i = 0; i < unnormalizedData.length; i++) {
		const id = unnormalizedData[i].id

		result.allIds.push(id)
		result.byId[id] = unnormalizedData[i]
	}
...
/* Комментарий
 * Действия:
 * В первую очередь создал тиип для входных и выходных данных.
 * далее с помощью цикла пройтись по входному массиву создавая при этом выходную структуру,
 * для этого потребовалось проинициализировать выходной параметр.
 * 
 * 
 * 
 */